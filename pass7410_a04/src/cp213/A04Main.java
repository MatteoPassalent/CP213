...
public class A04Main {
	 ...
	 */
	public static final String FILENAME = "pass7410_a04\\decline.txt";
	/**
	...
	private static void characterTable(final BST<Character> tree) {
		...
		for (final CountedItem<Character> data : array) {
			...
			System.out.format("%4s %,8d %7.2f%n", data.getValue(), count, percent);
		}
		return;
	}
	...
	private static int retrieve(final BST<Character> tree, final Scanner fileScan) {
		while (fileScan.hasNext()) {
			String word = fileScan.next().toUpperCase();
			for (int i = 0; i < word.length(); i++) {
				char letter = word.charAt(i);
				if (Character.isLetter(letter)) { // Check if the character is a letter
					CountedItem<Character> data = new CountedItem<>(Character.toUpperCase(letter));
					tree.retrieve(data);
				}
			}
		}
	...
	private static void testAVL() {
		...
		System.out.println("valid {true}: " + source.isValid());
	...
	private static void testBST() {
		...
		CountedItem<Integer> four = new CountedItem<>(4);

		final BST<Integer> target = new BST<>();
		for (Integer i : testData) {
			CountedItem<Integer> data = new CountedItem<>(i);
			target.insert(data);
		}
		...
		System.out.println("valid {true}: " + source.isValid());
		System.out.println("Contains 2 {true}: " + source.contains(source.root.getRight().getdata()));
		System.out.println("Contains 1 {true}: " + source.contains(source.root.getdata()));
		System.out.println("Contains 3 {true}: " + source.contains(source.root.getRight().getRight().getdata()));
		System.out.println("Contains 4 {false}: " + source.contains(four));
		System.out.println("source == target {true} " + source.equals(target));
		System.out.println("retrieve 1 {1 : 1}: " + source.retrieve(source.root.getdata()));
		System.out.println("retrieve 2 {2 : 1}: " + source.retrieve(source.root.getRight().getdata()));
		System.out.println("retrieve 3 {3 : 1}: " + source.retrieve(source.root.getRight().getRight().getdata()));
		System.out.println("retrieve 4 {null}: " + source.retrieve(four));
		System.out.println(LINE);
		System.out.println();
	}
